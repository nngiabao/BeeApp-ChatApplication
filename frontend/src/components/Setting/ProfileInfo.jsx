...
export default function ProfileInfo({ setActiveTicket }) {
    ...
    useEffect(() => {
        if (showTickets) {
            fetch(`${import.meta.env.VITE_API_URL}/supports/user/${user.id}`)
                .then(res => res.json())
    ...
    const handleSendTicket = async () => {
        ...
        try {
            await fetch(`${import.meta.env.VITE_API_URL}/supports`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                            ...
                            <button
                                onClick={async () => {
                                    ...
                                    const response = await fetch(
                                        `${import.meta.env.VITE_API_URL}/supports/${selectedTicket.id}/reply?senderId=${user.id}&senderType=USER&message=${encodeURIComponent(replyMessage)}`,
                                        { method: "POST" }
                                    );
                                    if (response.ok) {
                                        alert("Reply sent!");
